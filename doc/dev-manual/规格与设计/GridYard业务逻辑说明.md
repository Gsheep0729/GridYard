...
- 本阶段消息先存内存，落地本地 SQLite 与跨设备漫游分别属于后续数据层与服务端阶段。
...
SQLite 本地历史不依赖登录，优先于服务端开发完成。保存全局记录 ID、方向、文件显示名、总大小、状态、对端设备信息、开始/结束时间和错误信息。默认不保存或同步敏感源路径。QML 不直接查询数据库，由 Repository 通过应用逻辑层提供列表模型。
