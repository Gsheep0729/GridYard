...
本文档早期内容同时描述了 V1.0 与完整混合架构，部分 Type 码、Widgets 示例和阶段顺序已不再代表当前实现。自 2026-06-17 起，以以下决策为准：
...
- 在线文本聊天优先用 P2P 实现：在线设备之间的消息走现有 TCP + FrameCodec 通道直发，不依赖服务端与登录；文本 Type 码在 V1.0 低位段（`0x05xx`）扩展。
- LocalSend 作为产品和协议语义参考，重点借鉴版本/能力声明、主动发现回应、会话 token、部分接受、限流和安全降级。
- SQLite 统一承载本地传输历史与聊天记录；登录后可选择同步必要元数据。
- GridYard-Server 不中转 P2P 文件内容，也不替代在线 P2P 直发，只负责身份鉴权、对话历史与传输记录漫游、以及接收方离线时的消息暂存兜底。
- V2.0 Type 码统一从 `0x1000` 起分配；低位区间保留 V1.0 P2P 文件传输与在线聊天 Type。
...
| F-111 | P2P 在线文本聊天   | 在线设备之间通过现有 P2P 通道直接收发文本消息，无需账号与服务端；对端离线时不可发送         | 高     |

### 3.2 C/S 局域网即时通讯子系统

> 在线单聊由 §3.1 的 P2P 在线文本聊天（F-111）直接提供，不依赖服务端。本子系统在账号体系之上补足 P2P 无法覆盖的能力：登录鉴权、接收方离线时的消息暂存兜底，以及对话历史与传输记录的跨设备漫游。
...
| `0x0501`           | `kTypeChatText`         | P2P 在线文本消息             | JSON           |
| `0x0502`           | `kTypeChatAck`          | 在线文本消息送达回执         | JSON           |

`0x05xx` 段为 V1.0 P2P 在线聊天预留，与文件传输共用 TCP 通道，由首帧 Type 区分连接用途。V2.0 登录、漫游与离线消息 Type 从 `0x1000` 起分配。协议升级还应增加版本、能力、会话 token、错误码和帧长度上限；这些字段优先在现有 TLV 上扩展，不以更换 HTTP 或文本协议为前提。
...
> 本章是立项阶段的原始路线图，保留用于追溯设计变化。其中 Qt Designer、`tw_` 控件、`LocalDbManager` 单例和“先做登录”等安排不再作为当前实施依据；当前开发顺序以《鸽邮(GridYard)——软件开发阶段计划》v1.2 为准。
...
客户端保留 P2P 核心（含 V1.0 的在线聊天 `ChatManager`），通过现有应用层边界增量接入以下服务端相关模块：
...
| `0x1020`    | `TYPE_MSG_SEND`         | 接收方离线时经服务端暂存消息 |
| `0x1021`    | `TYPE_MSG_RECV`         | 服务端向上线客户端投递离线/漫游消息 |
...
*本文档由"鸽邮 (GridYard)"项目团队整理，版本 v1.2，随开发进展持续更新。*
